...
function isValidAnswer(q,v){if(!v)return true;if(hasChoices(q))return q.choices.some((c,i)=>choiceId(c,i)===String(v));return v==="○"||v==="×"}
...
function answerLabel(q){if(!hasChoices(q))return answerOf(q);let hit=q.choices.find((c,i)=>choiceId(c,i)===answerOf(q));return hit?`${answerOf(q)}. ${choiceText(hit)}`:answerOf(q)}
...
function arr(e=examId,s=setId){let ex=data.exams.find(x=>x.id===e)||data.exams[0],se=ex.sets.find(x=>x.id===s)||ex.sets[0],list=(se.questions||[]),n=list.length,k=e+"::"+s;if(!store[k])store[k]={answers:Array(n).fill(""),graded:false,wrong:[],score:null,updated:null,review:[],manualFeedback:"",chatgptResult:null};if(store[k].answers.length!==n)store[k].answers=Array(n).fill("");store[k].answers=store[k].answers.map((v,i)=>isValidAnswer(list[i]||{},v)?v:"");if(n===0){store[k].graded=false;store[k].wrong=[];store[k].score=null;store[k].review=[]}if(!store[k].review)store[k].review=[];return store[k]}
...
function render(){if(!data)return;home();question();stats();graphs();calendar();reviews();registered();radar();renderTimerPanels();$("autoFeedbackBox").textContent=feedback();dueNotice()}
...
function question(){
  ...
  let choicesHtml="";
  if(hasChoices(q)){
    choicesHtml=`<div class="choices examChoices">${q.choices.map((c,ci)=>{
